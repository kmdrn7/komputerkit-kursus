                ...
                <div class="card">
                    ...
                    <div class="card-content">
                        ...
                        <div class="table-responsive" id="table--container">
							...
							@include('admin.upgrade_materi.tbl_upgrade')
                        </div>
                    </div>
                    <!-- end content-->
                </div>
					...
					<div class="row no-margin-top">
						<div class="col-md-12">
							<button type="button" class="btn btn-primary btn-upgrade-materi">
								<i class="fa fa-external-link"></i>&nbsp; Upgrade Materi
							</button>
	...
	<div class="modal fade" id="modalTambah" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
		<div class="modal-dialog" style="max-width: 1080px; width: 85%;">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					<h4 class="modal-title">Tambah Manual Materi Kursus</h4>
				</div>
				<div class="modal-body">
					<div class="row">
						<div class="col-md-12">
...
@section('content-js')
	<script type="text/javascript">
		...
		function fetch(id = '') {
			$('#loading').css('display', 'block');
			$('#DT_wrapper').remove('');
			axios.get('{{route('ajax.upmateri')}}', {params: {id_kursus: id}}).then(function (res) {
				$('#loading').css('display', 'none');
		...
		function fetchMateriKursus(id, id_user) {
			...
			axios.get('/admin/upgrade-materi/show/' + id).then(function (res) {
		...
		$(document).ready(function() {
			...
			$('.btn-upgrade-materi').click(function(event) {
				$('#modalLoading').modal('show');
				axios.get('{{ url('/admin/upgrade-materi/fetch_option/') }}', {
					params: {
			...
			$('.btn-tambah-materi').click(function(event) {
				$('#modalLoading').modal('show');
				axios.get('{{ url('/admin/upgrade-materi/show/') }}' + '/'+$('select[name="kursus"]').val(), {
					params: {
						from: 'modal',
						id_kursus: $('select[name="kursus"]').val(),
					}
			...
			$('#tambahkan').click(function(event) {
				...
				if ( checked.length < 1 ) {
					...
					axios.post('/admin/upgrade-materi/add', {
						id_detail_materi: checked ,
			...
			$('.btn-hapus').click(function(event) {
				...
				if ( checked.length < 1 ) {
					...
					$('#list-materi--container').html('');
					$('#loading-container').html('');
					axios.post('/admin/upgrade-materi/delete', { id_detail_materi: checked }).then(function(res) {
						console.log(res.data);
						fetchMateriKursus($('input[name="id_detail_kursus"]').val(), $('input[name="id_user"]').val());
					}, function(res) {
			...
			$('#upgrade').click(function(event) {
				...
				axios.post('{{ url('/admin/upgrade-materi/upgrade') }}', data).then(function(res) {
